...
calendarDIV = document.getElementById('calendar-left-side');

canScroll = true;
calendarDIV.onmousewheel = function(event, delta) {
	event.preventDefault(event);
	if(!canScroll){return}
		canScroll = false;
	event.deltaY > 0 &&  event.deltaY > 90 ? nextButt() : prevButt();
	setTimeout(function(){
		canScroll = true
	}, 300)
}
...
/* определяем разницу между текущей и выбранной датой */
...
var rotateTill;
/* крутим колесо пока не найдем нужную дату */
...
/* изменение даты в календаре при клике след пред событие */
function changeDate() {
